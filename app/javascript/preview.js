document.addEventListener('DOMContentLoaded', function(){
  // 新規投稿・編集ページのフォームを取得
  const albumForm = document.getElementById('new_album');
  ...
  const buildNewFileField = () => {
    ...
    newFileField.setAttribute('name', 'album[images][]');

    // 最後のfile_fieldを取得
    const lastFileField = document.querySelector('input[type="file"][name="album[images][]"]:last-child');
  ...
  // input要素を取得
  const fileField = document.querySelector('input[type="file"][name="album[images][]"]');
